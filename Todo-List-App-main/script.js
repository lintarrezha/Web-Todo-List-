...
let tasks = [
  {
    id: 1,
    task: 'Pergi ke super market',
    category: 'Shopping',
    completed: false,
  },
  {
    id: 2,
    task: 'Membaca buku tentang menejemen waktu',
    category: 'Personal',
    completed: false,
  },
  {
    id: 3,
    task: 'Menyiapkan presentasi untuk rapat',
    category: 'Work',
    completed: false,
  },
  {
    id: 4,
    task: 'Menyelesaikan tugas coding',
    category: 'Coding',
    completed: false,
  },
  {
...
window.onload = function () {
  ...
  if (loggedInUser) {
    document.querySelector('.welcome h1').innerText = `Hello ${loggedInUser}`;
  } else {
    // Jika tidak ada pengguna yang login, mungkin alihkan ke halaman login
...
// Define functions
const logout = () => {
  localStorage.removeItem('loggedInUser'); // Menghapus status login
...
function saveEditedTask() {
  ...
  const editedCategorySelect = document.getElementById('edit-category-select');
  ...
  if (taskIndex !== -1) {
    tasks[taskIndex].task = editedTaskInput.value.trim();
    tasks[taskIndex].category = editedCategorySelect.value;
    saveLocal();
    renderTasks();
    closeEditTaskModal();
  }
